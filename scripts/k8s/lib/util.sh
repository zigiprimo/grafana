...
# $GRAFANA_ROOT must be set
kube::util::find-binary-for-platform() {
  local -r lookfor="$1"
  local -r platform="$2"
  local locations=(
    "${GRAFANA_ROOT}/_output/bin/${lookfor}"
    "${GRAFANA_ROOT}/_output/dockerized/bin/${platform}/${lookfor}"
    "${GRAFANA_ROOT}/_output/local/bin/${platform}/${lookfor}"
    "${GRAFANA_ROOT}/platforms/${platform}/${lookfor}"
  )
  ...
  if [[ "${platform}" = "$(kube::util::host_platform)" ]]; then
    locations+=(
      "${GRAFANA_ROOT}/_output/local/go/bin/${lookfor}"
      "${GRAFANA_ROOT}/_output/dockerized/go/bin/${lookfor}"
    );
  fi
...
# Removes previously generated docs-- we don't want to check them in. $GRAFANA_ROOT
# must be set.
kube::util::remove-gen-docs() {
  if [ -e "${GRAFANA_ROOT}/docs/.generated_docs" ]; then
    # remove all of the old docs; we don't want to check them in.
    while read -r file; do
      rm "${GRAFANA_ROOT}/${file}" 2>/dev/null || true
    done <"${GRAFANA_ROOT}/docs/.generated_docs"
...
kube::util::group-version-to-pkg-path() {
  ...
  kube::util::read-array apidirs < <(
      cd "${GRAFANA_ROOT}/staging/src/k8s.io/api" || return 1 # make shellcheck happy
...
function kube::util::list_staging_repos() {
  (
    cd "${GRAFANA_ROOT}/staging/src/k8s.io" && \
