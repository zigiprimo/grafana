...
import { LogContextSupport, REF_ID_STARTER_LOG_ROW_CONTEXT } from './LogContextSupport';
    ...
    DataSourceWithQueryExportSupport<LokiQuery>
{
  ...
  maxLines: number;
  logContextSupport: LogContextSupport;
  ...
  constructor(
    ...
    this.logContextSupport = new LogContextSupport(this.languageProvider);
  }
  ...
  getLogRowContext = async (
    ...
    const { query, range } = await this.logContextSupport.prepareLogRowContextQueryTarget(
      row,
      limit,
      direction,
      origQuery
  ...
  getLogRowContextUi(row: LogRowModel, runContextQuery: () => void): React.ReactNode {
    return this.logContextSupport.getLogRowContextUi(row, runContextQuery);
  }
