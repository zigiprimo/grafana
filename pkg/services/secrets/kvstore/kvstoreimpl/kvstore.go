...
func ProvideService(
	...
	cfg *setting.Cfg,
) (kvstoreService.SecretsKVStore, error) {
	var logger = log.New("secrets.kvstore")
	var store kvstoreService.SecretsKVStore
...
func With(kv kvstoreService.SecretsKVStore, orgId int64, namespace string, typ string) *FixedKVStore {
	return &FixedKVStore{
		kvStore:   kv,
		OrgId:     orgId,
		Namespace: namespace,
...
type FixedKVStore struct {
	kvStore   kvstoreService.SecretsKVStore
	OrgId     int64
	Namespace string
	Type      string
}
...
func (kv *FixedKVStore) Keys(ctx context.Context) ([]kvstoreService.Key, error) {
