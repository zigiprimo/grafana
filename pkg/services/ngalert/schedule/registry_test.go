...
func TestSchedule_alertRuleInfo(t *testing.T) {
	...
	t.Run("when rule evaluation is not stopped", func(t *testing.T) {
		t.Run("update should send to updateCh", func(t *testing.T) {
			factory := RuleFactoryForTests()
			r := factory(context.Background())
...
func RuleFactoryForTests() RuleFactoryFunc {
	...
	return NewRuleFactory(nil, nil, nil, clock.New(), m.GetSchedulerMetrics(), log.NewNopLogger(), tracing.InitializeTracerForTest())
}
