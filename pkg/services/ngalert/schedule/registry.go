...
// Fingerprint returns a fingerprint that composes two hashes: models.AlertRule StateFingerprint and hash of the folder title.
func (r ruleWithFolder) Fingerprint() fingerprint {
	sum := fnv.New64()
	// avoid allocation when converting string to byte slice
	sum.Write(*(*[]byte)(unsafe.Pointer(&r.folderTitle)))
	return fingerprint(string(r.rule.StateFingerprint) + fmt.Sprintf("%016x", sum.Sum64()))
}
