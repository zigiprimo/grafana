...
// SetRequestHandlerHook allows for a function to be specified which wraps all
// request handlers. The hook is invoked after all other middleware is invoked
// (authentication, instrumentation).
func (ng *AlertNG) SetAPIRequestHandlerHook(hook api.RequestHandlerFunc) {
	ng.api.RequestHandlerHook = hook
}
