...
func (alertRule *AlertRule) PreSave(timeNow func() time.Time) error {
	...
	alertRule.StateFingerprint = CalculateAlertRuleFingerprint(alertRule)
	return nil
}
...
// CalculateAlertRuleFingerprint calculates a fingerprint that includes all fields except rule's Version and Update timestamp.
func CalculateAlertRuleFingerprint(rule *AlertRule) Fingerprint {
	sum := fnv.New64()
	...
	return Fingerprint(fmt.Sprintf("%016x", sum.Sum64()))
}
