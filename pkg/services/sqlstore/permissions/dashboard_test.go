...
func TestIntegration_DashboardFilter(t *testing.T) {
	...
	tests := []testCase{
		...
		{
			desc:       "User with self contained permissions should be able to view all dashboards with wildcard scope",
			permission: dashboards.PERMISSION_VIEW,
			permissions: []accesscontrol.Permission{
				{Action: dashboards.ActionDashboardsRead, Scope: dashboards.ScopeDashboardsAll},
			},
		...
		{
			desc:       "User with self contained permissions should be able to view all dashboards with folder wildcard scope",
			permission: dashboards.PERMISSION_VIEW,
			permissions: []accesscontrol.Permission{
				{Action: dashboards.ActionDashboardsRead, Scope: dashboards.ScopeFoldersAll},
			},
		...
		{
			desc:       "User with self contained permissions should be able to view a subset of dashboards with dashboard scopes",
			permission: dashboards.PERMISSION_VIEW,
			permissions: []accesscontrol.Permission{
				{Action: dashboards.ActionDashboardsRead, Scope: "dashboards:uid:110"},
				{Action: dashboards.ActionDashboardsRead, Scope: "dashboards:uid:40"},
		...
		{
			desc:       "User with self contained permissions should be able to view a subset of dashboards with dashboard action and folder scope",
			permission: dashboards.PERMISSION_VIEW,
			permissions: []accesscontrol.Permission{
				{Action: dashboards.ActionDashboardsRead, Scope: "folders:uid:8"},
				{Action: dashboards.ActionDashboardsRead, Scope: "folders:uid:10"},
		...
		{
			desc:       "User with self contained permissions should be able to view all folders with folder wildcard",
			permission: dashboards.PERMISSION_VIEW,
			permissions: []accesscontrol.Permission{
				{Action: dashboards.ActionFoldersRead, Scope: "folders:uid:*"},
			},
		...
		{
			desc:       "User with self contained permissions should be able to view a subset folders",
			permission: dashboards.PERMISSION_VIEW,
			permissions: []accesscontrol.Permission{
				{Action: dashboards.ActionFoldersRead, Scope: "folders:uid:3"},
				{Action: dashboards.ActionFoldersRead, Scope: "folders:uid:6"},
		...
		{
			desc:       "User with self contained permissions should return folders and dashboard with 'edit' permission",
			permission: dashboards.PERMISSION_EDIT,
			permissions: []accesscontrol.Permission{
				{Action: dashboards.ActionDashboardsCreate, Scope: "folders:uid:3"},
				{Action: dashboards.ActionDashboardsWrite, Scope: "dashboards:uid:33"},
		...
		{
			desc:       "User with self contained permissions should return the dashboards that the User has dashboards:write permission on in case of 'edit' permission",
			permission: dashboards.PERMISSION_EDIT,
			permissions: []accesscontrol.Permission{
				{Action: dashboards.ActionFoldersRead, Scope: "folders:uid:3"},
				{Action: dashboards.ActionDashboardsRead, Scope: "dashboards:uid:31"},
		...
		{
			desc:       "User with self contained permissions should return the folders that the User has dashboards:create permission on in case of 'edit' permission",
			permission: dashboards.PERMISSION_EDIT,
			permissions: []accesscontrol.Permission{
				{Action: dashboards.ActionDashboardsCreate, Scope: "folders:uid:3"},
				{Action: dashboards.ActionFoldersRead, Scope: "folders:uid:4"},
		...
		{
			desc:       "User with self contained permissions should return folders that users can read alerts from",
			permission: dashboards.PERMISSION_VIEW,
			queryType:  searchstore.TypeAlertFolder,
			permissions: []accesscontrol.Permission{
				{Action: accesscontrol.ActionAlertingRuleRead, Scope: "folders:uid:3"},
	...
	for _, tt := range tests {
		t.Run(tt.desc, func(t *testing.T) {
			...
			var filter permissions.PermissionFilter
			if tt.oldQuery {
				filter = permissions.NewAccessControlDashboardPermissionFilter(usr, tt.permission, tt.queryType, featuremgmt.WithFeatures(), false)
			} else {
...
func queryWithFilter(store db.DB, filter permissions.PermissionFilter) (int, error) {
	var result int
...
func TestIntegration_DashboardNestedPermissionFilter(t *testing.T) {
	testCases := []struct {
		...
		{
			desc:       "User with self contained permissions should be able to view dashboards under inherited folders if nested folders are enabled",
			queryType:  searchstore.TypeDashboard,
			permission: dashboards.PERMISSION_VIEW,
			permissions: []accesscontrol.Permission{
				{Action: dashboards.ActionDashboardsRead, Scope: "folders:uid:parent"},
		...
		{
			desc:       "User with self contained permissions should not be able to view dashboards under inherited folders if nested folders are not enabled",
			queryType:  searchstore.TypeDashboard,
			permission: dashboards.PERMISSION_VIEW,
			permissions: []accesscontrol.Permission{
				{Action: dashboards.ActionDashboardsRead, Scope: "folders:uid:parent"},
		...
		{
			desc:       "User with self contained permissions should be able to view inherited folders if nested folders are enabled",
			queryType:  searchstore.TypeFolder,
			permission: dashboards.PERMISSION_VIEW,
			permissions: []accesscontrol.Permission{
				{Action: dashboards.ActionFoldersRead, Scope: "folders:uid:parent"},
		...
		{
			desc:       "User with self contained permissions should not be able to view inherited folders if nested folders are not enabled",
			queryType:  searchstore.TypeFolder,
			permission: dashboards.PERMISSION_VIEW,
			permissions: []accesscontrol.Permission{
				{Action: dashboards.ActionFoldersRead, Scope: "folders:uid:parent"},
		...
		{
			desc:       "User with self contained permissions should be able to view inherited dashboards and folders if nested folders are enabled",
			permission: dashboards.PERMISSION_VIEW,
			permissions: []accesscontrol.Permission{
				{Action: dashboards.ActionFoldersRead, Scope: "folders:uid:parent"},
				{Action: dashboards.ActionDashboardsRead, Scope: "folders:uid:parent"},
		...
		{
			desc:       "User with self contained permissions should not be able to view inherited dashboards and folders if nested folders are not enabled",
			permission: dashboards.PERMISSION_VIEW,
			permissions: []accesscontrol.Permission{
				{Action: dashboards.ActionFoldersRead, Scope: "folders:uid:parent"},
				{Action: dashboards.ActionDashboardsRead, Scope: "folders:uid:parent"},
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			if !tc.selfContained {
				...
				usr = &user.SignedInUser{OrgID: orgID, AuthenticatedBy: login.ExtendedJWTModule, Permissions: map[int64]map[string][]string{orgID: accesscontrol.GroupScopesByAction(tc.permissions)}}
			}
			...
			if tc.oldQuery {
				...
				filter = permissions.NewDashboardFilter(usr, tc.permission, tc.queryType, tc.features, recursiveQueriesAreSupported)
			}
			result, err := queryTitleWithFilter(db, filter)
			require.NoError(t, err)
...
func queryTitleWithFilter(store db.DB, filter permissions.PermissionFilter) ([]string, error) {
	var result []string
	err := store.WithDbSession(context.Background(), func(sess *sqlstore.DBSession) error {
		query := strings.Builder{}
		params := []interface{}{}

		with, withPaparams := filter.With()
		params = append(params, withPaparams...)
		query.WriteString(with)
		query.WriteString("SELECT dashboard.title FROM dashboard ")
		query.WriteString(" LEFT OUTER JOIN dashboard AS folder ON folder.id = dashboard.folder_id ")
		join := filter.LeftJoin()
		if join != "" {
			query.WriteString(" LEFT OUTER JOIN ")
		...
		/*
			groupBy, groupByParams := filter.GroupBy()
			if groupBy != "" {
		...
		*/
		...
		query.WriteString(where)
		params = append(params, whereParams...)
		err := sess.SQL(query.String(), params...).Find(&result)
		return err
	})
	return result, err
}
...
func setupNestedTest(t *testing.T, usr *user.SignedInUser, perms []accesscontrol.Permission, orgID int64, features featuremgmt.FeatureToggles) db.DB {
	...
	err = db.WithDbSession(context.Background(), func(sess *sqlstore.DBSession) error {
		...
		for i := range perms {
			kind, attribute, identifier := perms[i].SplitScope()
			perms[i].RoleID = role.ID
			perms[i].Created = time.Now()
			perms[i].Updated = time.Now()
			perms[i].Kind = kind
			perms[i].Attribute = attribute
			perms[i].Identifier = identifier
		}
		if len(perms) > 0 {
