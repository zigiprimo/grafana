...
type Manager interface {
	...
	GetPluginStates() map[PluginInfo]Status
}
...
var statusMap = map[Status][]Status{
	StatusNone:         {StatusRequested, StatusDiscovered, StatusError},
	...
	StatusUninstalled:  {StatusRequested, StatusError},
}
...
func (s *Service) GetPluginStates() map[PluginInfo]Status {
	states := make(map[PluginInfo]Status)
	for info, st := range s.states {
		states[info] = st.curStatus
	}
